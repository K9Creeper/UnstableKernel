...
extern "C" uint32_t kmalloc_(uint32_t size, uint8_t align, uint32_t * physAddress) {
  if (Kernel::Memory::KHeap::bInitialized) {} else {
...
static void Expand(uint32_t new_size) {
  ...
  while (i < new_size) {
    ...
    Kernel::Memory::Paging::AllocateFrame(
      Kernel::Memory::Paging::GetPageEntry(Kernel::Memory::KHeap::heap -> startAddress + i, 1, Kernel::Memory::Paging::kernelDirectory),
...
static uint32_t Contract(uint32_t new_size) {
  ...
  while (new_size < i) {
    ...
    Kernel::Memory::Paging::FreeFrame(
      Kernel::Memory::Paging::GetPageEntry(Kernel::Memory::KHeap::heap -> startAddress + i, 0, Kernel::Memory::Paging::kernelDirectory));
...
void Free(void * p) {
  ...
  if (test_footer -> magic == 0xDEADBEEF &&
  ...
  if (test_header -> magic == 0xDEADBEEF && test_header -> is_hole) {
    // Increase our size
...
{
	return ( ( Kernel::Memory::KHeap::Header * ) a ) -> size <
	       ( ( Kernel::Memory::KHeap::Header * ) b ) -> size ? 1 : 0;
}
...
void Kernel::Memory::KHeap::Init(uint32_t start, uint32_t end, uint32_t max, bool supervisor, bool readonly) {
  ...
  Kernel::Memory::KHeap::heap->index = orderedArray_place( ( void * ) start, 0x20000, &header_t_less_than );

  start += sizeof(void * ) * 0x20000;
